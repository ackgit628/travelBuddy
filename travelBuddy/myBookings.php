...
<?php
    ...
    if (isset($_SESSION["usertype"])) {                                             //select bookings for tour operators
        ...
        $sql1 = "SELECT * FROM bookings WHERE endd>=? ORDER BY start ASC";          //check for bookings
        ...
        $sql1 = "SELECT * FROM bookings WHERE uname=? AND endd>=? ORDER BY start ASC";                 //check for bookings
        $stmt = mysqli_stmt_init($conn);                                            //prepared statement
        if (!mysqli_stmt_prepare($stmt, $sql1)) {
            header("Location:../index.php?error=sqlerror");
            exit();
        ...
        mysqli_stmt_bind_param($stmt, "ss", $uname, $today);
    }
    ...
    if ($bookingCheck > 0):
        ...
        echo "<h3 style=\"padding: 20px;\">".$bookingCheck." Upcoming Bookings found</h3>";
        ...
        endwhile;
            ...
            echo "<h3 style=\"padding: 20px;\">No Upcoming Bookings found</h3>";
...
<div align="center">
    ...
    <?php
        if (isset($_GET["error"]) && $_GET["error"] == "empty") {
            echo "<p align=\"center\">Select Booking to cancel!<p>";
        }

        if ($bookingCheck > 0):
    ?>
    <form id="cancelBooking" action="inc/cancelBooking_inc.php" method="post">
        ...
        <button type="submit" name="submit-cancel" class="cancelbtn">Delete</button>
    </form>
    <?php endif; ?>
